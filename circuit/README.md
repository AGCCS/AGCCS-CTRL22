...
However, if you have installed the ESP32 with the provided firmware `demesh`, there is an even more convenient and --- given that we are running on mains power --- more safe alternative. On power-on, `demesh` reads a configurable IO pin (default IO15) to choose to boot into *target-development mode*. In this mode it will not setup a wifi mesh network but instead act as an access point and provide a transparent telnet passthrough of the AVR serial port TX0/RX0 incl. the Optiboot boot loader. Thus, we can develop/test the AVR firmware largely independently from the more involved wifi mesh but still have the convenience of "no wires from inside the charging station directly to the computer".
