...
void button_task(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        xEventGroupClearBits(button->event_group, BUTTON_PLUSED_BIT | BUTTON_RELESED_BIT);

	// tm: press/release semantics
        event = xEventGroupWaitBits(button->event_group, BUTTON_PLUSED_BIT | BUTTON_RELESED_BIT , pdFALSE, pdFALSE,  1000 / portTICK_PERIOD_MS);
	if(event & (BUTTON_PLUSED_BIT | BUTTON_RELESED_BIT) ) {
	...
	/*
        // original: press/hold semantics
        event = xEventGroupWaitBits(button->event_group, BUTTON_PLUSED_BIT | BUTTON_RELESED_BIT , pdFALSE, pdFALSE, portMAX_DELAY)
        if ((event & BUTTON_PLUSED_BIT) != 0)
        {
            xEventGroupClearBits(button->event_group, BUTTON_PLUSED_BIT);
            event = xEventGroupWaitBits(button->event_group, BUTTON_PLUSED_BIT, pdFALSE, pdFALSE, button->Countime / portTICK_PERIOD_MS);
            vTaskDelay( button->Countime / portTICK_PERIOD_MS );
